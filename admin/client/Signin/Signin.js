...
var SigninView = React.createClass({
	...
	handleSubmit (e) {
		...
		if (!this.state.email || !this.state.password) {
			return this.displayError('Por favor introduza um email e password para entrar.');
		}
		...
		xhr({
			...
			if (err || body && body.error) {
				return body.error === 'invalid csrf'
					? this.displayError('Foi encontrado um erro, por favor faça refresque o browser.')
					: this.displayError('O email e/ou password não são validos.');
			} else {
		...
		return (
			<div className="auth-wrapper">
				...
				<div className="auth-footer">
					...
					<a href="http://google.com" target="_blank" title="Lana">MSN</a>
				</div>
			</div>
		);
	},
